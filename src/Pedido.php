...
class Pedido{
    ...
    public function registrarDetalle($_params){
        $sql = "INSERT INTO `detalle_pedidos`(`pedido_id`, `accesorio_id`, `precio`, `cantidad`) 
        VALUES (:pedido_id,:accesorio_id,:precio,:cantidad)";

        $resultado = $this->cn->prepare($sql);

        $_array = array(
            ":pedido_id" => $_params['pedido_id'],
            ":accesorio_id" => $_params['accesorio_id'],
    ...
    public function mostrarDetallePorIdPedido($id)
    {
        $sql = "SELECT
                ...
                INNER JOIN accesorios ac ON ac.id= dp.accesorio_id
